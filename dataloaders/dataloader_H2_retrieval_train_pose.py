...
class H2_DataLoader_train_pose(Dataset):
    """H2 dataset loader."""
    def __init__(
            self,
            subset,
        ...
        for sentance_id in sentance_ids:
            for i_video in range(len(captions[sentance_id])):
                video_path=os.path.join(self.features_path, captions[sentance_id][i_video]['new_video_name']+'.pkl')
                video_RGB_path=os.path.join(os.path.join(self.features_RGB_path, subset), captions[sentance_id][i_video]['new_video_name']+'.pkl')
    ...
    def _get_text(self, ids):
        ...
        for i, sentance_id in enumerate(choice_sentance_ids):
            ...
            sample["pairs_text_aug"] = torch.from_numpy(pairs_text_aug)
            sample["pairs_mask_aug"] = torch.from_numpy(pairs_mask_aug)

        return sample, choice_sentance_ids[0]
    ...
    def _get_body_pose(self, video_data, ori_img_size, total_frame_list):
        ...
        if len(frame_list) > self.max_length_frames:
            interval = math.ceil(float(len(frame_list) / self.max_length_frames))
...
def H2_train_pose_collate_fn(batch, padding=6):
    '''
