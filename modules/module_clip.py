...
class CrossAttentionBlock(nn.Module):
    ...
    def attention(self, trg, enc_src, src_mask, attn_mask):
        video_mask = src_mask.to(device=trg.device)==1 if src_mask is not None else None
        return self.attn(trg, enc_src, enc_src, key_padding_mask=video_mask,need_weights=False)[0]


    def forward(self, trg, enc_src, src_mask,attn_mask=None):
        
        trg=trg.permute((1, 0, 2))
        enc_src=enc_src.permute((1, 0, 2))

        x = self.attention(self.ln_1(trg), self.ln_2(enc_src), src_mask,attn_mask)
        x = self.ln_3(x)
        x = x.permute((1, 0, 2))
...
class VTransformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask = None):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[VResidualAttentionBlock(width, heads, attn_mask) for _ in range(layers)])
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        self.conv3 = nn.Conv2d(in_channels=1024, out_channels=width, kernel_size=1, stride=1, bias=False)
...
class FeatureTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, feature_len: int, input_size: int,
                 linear_patch: str = '2d',):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim

        self.conv1 = nn.Conv2d(in_channels=input_size, out_channels=width, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, x: torch.Tensor, video_mask, video_frame=-1):
        ...
        if self.linear_patch == '3d':
            ...
            #conv core [768, 3, 32, 32] -> [768, 1024, 1, 1]
            #[B, 1024, feature_len, 1] -> [B, 768, feature_len, 1]
            x = self.conv1(x)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # self.cross_atten_vt=CrossAttentionBlock(d_model=embed_dim,n_head=8)
        # self.cross_atten_tv=CrossAttentionBlock(d_model=embed_dim,n_head=8)
        ...
        self.logit_scale_first_softmax = nn.Parameter(torch.ones([]))
        self.logit_scale_sec_softmax = nn.Parameter(torch.ones([]))
    ...
    def encode_image(self, image, return_hidden=False,video_mask=None, video_frame=-1):
        ...
        if return_hidden:
            return video_mask, hidden

        return x

    def encode_feature(self, feature, return_hidden=False):
        x=self.mlp(feature)
        return x
    ...
    def encode_text(self, text, return_hidden=False):
        ...
        x = x.permute(1, 0, 2)  # LND -> NLD B,20,512

        hidden = self.ln_final(x).type(self.dtype) @ self.text_projection
        text_lenth = text.argmax(dim=-1)
        text_mask=torch.ones((hidden.shape[0],hidden.shape[1]),device=hidden.device)
        for i in range(hidden.shape[0]):
            text_mask[i,text_lenth[i] + 1:]=0
        # text_lenth = text.argmax(dim=-1)
        # for i in range(hidden.shape[0]):
        #     hidden[i, text_lenth[i] + 1:, :] = 0
        # x.shape = [batch_size, n_ctx, transformer.width]
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        x = hidden[torch.arange(hidden.shape[0]), text.argmax(dim=-1)]

        if return_hidden:
            return text_mask,hidden

        return x

    def forward(self, image, text):
        image_features = self.encode_feature(image)
