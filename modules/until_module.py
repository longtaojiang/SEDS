...
class PreTrainedModel(nn.Module):
    ...
    def init_preweight(cls, model, state_dict, prefix=None, task_config=None):
        old_keys = []
        new_keys = []
        if task_config.not_load_visual:
            for key in state_dict.keys():
                new_key = None
                if 'visual' in key:
                    new_key = key.replace('visual', 'no_load_visual')
                if new_key:
                    old_keys.append(key)
...
class KL(nn.Module):
    def __init__(self, logit_scale=0.01):
        super(KL, self).__init__()
        self.logit_scale = logit_scale

    def forward(self, sim_matrix0, sim_matrix1):
        logpt0 = F.log_softmax(sim_matrix0/self.logit_scale, dim=-1)
        logpt1 = F.softmax(sim_matrix1/self.logit_scale, dim=-1)
        kl = F.kl_div(logpt0, logpt1, reduction='batchmean')
        return kl
